...
var zoom_reporting_top = '105px';
if (location.href.toLowerCase().indexOf('blankmode=1') >= 0) {
    //zoom_workspace_top = '0px';
    $('.zoom-reporting').css('top', zoom_reporting_top);
...
window.addEventListener('scroll', function() {
    ...
    if (scrollPos > 105) {
            $('.zoom-reporting').css('position', 'fixed');
            $('.zoom-reporting').css('top', '0');
    } else {
        $('.zoom-reporting').css('position', 'absolute');
        $('.zoom-reporting').css('top', zoom_reporting_top);
    }
});
