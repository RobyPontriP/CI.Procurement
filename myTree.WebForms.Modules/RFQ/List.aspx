<%@ Page MasterPageFile="~/Procurement.Master" Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="myTree.WebForms.Modules.RFQ.List" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="AppBody" runat="server">
        ...
        <div class="floatingBox">
            <div class="container-fluid">
                ...
                <div class="control-group last">
                    <% if(isEditable)
                        { %>
                    <button type="button" id="btnCreate" class="btn btn-success">Create new RFQ</button>
                    <% } %>
                </div>
                <div class="control-group last">
                    <table id="tblRFQ" class="table table-bordered" style="border: 1px solid #ddd">
                        <thead>
                            <tr>
                                <th style="width:15%;">RFQ code</th>
    ...
    <script>
        ...
        $(document).on("click", "#btnCreate", function () {
            location.href = "Wizard.aspx";
        });
        
        $(document).ready(function () {
            $("#RFQStatus").append("<b><i>RFQ status:</i></b><br/>");
            $.each(listStatus, function (i, x) {
                if (x.status_id != "-1") {
                    $("#RFQStatus").append("<span style='background-color:" + x.color_code + "' class='legend-block'>&nbsp;</span>&nbsp;<i>" + x.status_name + "</i><br/>");
                }
            });
           ...
           // generateComboGroup(cboOffice, listOffice, "office_id", "office_name", "hub_option", true);
                ...
                "aoColumns": [
                    {
                        ...
                        ,"mRender": function (d, type, row) {
                            var html = '<a href="<%=Page.ResolveUrl("detail.aspx?id=' + row.id + '")%>" title="View detail">' + row.rfq_no + '</a>';
                            return html;
                        }
                    },
                    { "mDataProp": "created_date" },
                    { "mDataProp": "vendor_name" },// as supplier name
                    ...
                    { "mDataProp": "status" },
                    {
                        ...
                        , "visible": isAdmin
                        , "mRender": function (d, type, row) {
                            var html = '<input name="rfq_no" type="hidden" value="' + row.rfq_no + '"/><input name="id" type="hidden" value="' + row.id + '"/>';
            ...
            $('#tblRFQ_filter input').unbind();
            $('#tblRFQ_filter input').bind('keyup', function (e) {
                if (e.keyCode == 13) {
                    RFQTable.search(this.value).draw();	
                }
            });	
        ...
        $(document).on("click", ".btnCancel", function () {
            _id = $(this).closest("td").find("input[name='id']").val();
            rfq_no = $(this).closest("td").find("input[name='rfq_no']").val();
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (output.result != "success") {
                        ...
                        alert(rfq_no + " has been cancelled successfully");
                        blockScreen();
